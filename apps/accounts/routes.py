...
async def get_users_list(db: AsyncSession = Depends(get_async_db)):
    return await handle_get_user(db)

@router.post('/create-department')
async def create_department(request: CreateDepartmentSchema, db: AsyncSession = Depends(get_async_db)):
    return await handle_create_department(request, db)

@router.get('/get-departments-list')
async def get_departments_list(db: AsyncSession = Depends(get_async_db)):
    return await handle_get_department(db)